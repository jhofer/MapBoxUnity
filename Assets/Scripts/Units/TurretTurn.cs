...
public class TurretTurn : MonoBehaviour
{
	...
	void LateUpdate ()
	{
		...
		dirToTarget = (aimSystem.nearestTarget.transform.position - HorizontalAxis.transform.position);
		...
		float xAngle=0;
		if(yClamped==yAngle){
			dirToTarget = (aimSystem.nearestTarget.transform.position - VerticalAxis.transform.position);
